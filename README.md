...
A tool for solving all of the non-environment puzzles in the video game
[The Witness](http://store.steampowered.com/app/210970/). Input a puzzle on the
grid, hit **Solve**, and the simplest path is drawn from start to exit. You can
also show only a fraction of the solution if you just want a hint.

> ⚠️ This repo contains a **modernized React + Vite** UI on top of the original
> branch-and-bound solver. The solver algorithm itself is unchanged.
...
This is a static Vite SPA, deployable to Vercel with zero extra setup.

1. Push this repo to GitHub.
2. In Vercel, **New Project → Import** the repo.
3. Vercel auto-detects Vite. The included [`vercel.json`](./vercel.json) pins the
   settings explicitly:
...
> Puzzle sharing uses the URL **hash** (`#...`), so it works on any static host
> without server-side routing config. Links generated by the old (jQuery) version
> still open correctly here, and vice-versa — the encoding format is unchanged.

## Tech stack

- **UI:** React 18, Vite 5
- **Solver:** vanilla JS (wrapped as ES modules in `src/solver/`)
- **Testing:** Vitest + Testing Library (43 tests)
- **Styling:** plain CSS with CSS variables (`src/styles/theme.css`)

### Project layout

```
src/
  solver/        # the original solver, wrapped as ES modules (algorithm untouched)
  lib/           # pure helpers: geometry, tetris, puzzleFactory, solverBridge,
                 #   solution highlights, urlCodec, constants
  hooks/         # usePuzzle (undo/redo reducer), useSolver
  components/    # App, PuzzleGrid, GridNode/Edge/Cell, EditorPopup, Toolbar
  styles/        # theme.css
